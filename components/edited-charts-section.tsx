...
import { useEditedCharts } from "@/hooks/use-chart-queries";
...
export default function EditedChartsSection() {
  const { data: charts = [], isLoading, error } = useEditedCharts();
      ...
      <Card className="p-6 bg-yellow-50 border border-yellow-200 dark:bg-yellow-950/20 dark:border-yellow-800">
        <div className="space-y-2">
          ...
          <p className="text-sm text-yellow-700 dark:text-yellow-300">
            {error instanceof Error ? error.message : "Không thể tải danh sách gia phả. Vui lòng thử lại sau."}
          </p>
        </div>
      </Card>
    );
